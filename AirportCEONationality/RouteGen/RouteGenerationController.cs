...
public class RouteGenerationController : MonoBehaviour
{
	...
	// Caching
	private Dictionary<Country, List<Airport>> airportsByCountry = new();
	private Dictionary<Country, SortedSet<RouteContainer>> largeRoutesByCountry = new();

	// Im going to let myself access this, but Im not sure if it is smart to let anyone else (hence not public)
	internal SortedSet<RouteContainer> RouteContainers { get; private set; }
	...
	// Mod loader calls this for setup!
	public IEnumerator Setup()
	{
		CoroutineEventDispatcher.GetTextUpdater()("AirportCEO Tweaks - Preloading Route Generation", 35);
	...
	public List<Airport> GetAirportsInCountry(Country country)
	{
		...
		List<Airport> airportResults = new();

		foreach (Airport airport in airports)
		{
			if (airport.Country.Equals(country))
			{
	...
	public SortedSet<RouteContainer> GetRoutesToLargeAirportsInCountry(Country country)
	{
		...
		{
			return largeRoutesByCountry[country];
		}
		...
		if (domesticAirportsInCountry.Count < LargeAirportsToGenerateInCountry)
		{
			...
			{
				result.Add(new RouteContainer(new Route(airport.id, PlayerAirport.id,
		...
		foreach (Airport airport in domesticAirportsInCountry)
		{
			if (airport.paxSize < Enums.GenericSize.Large && airport.cargoSize < Enums.GenericSize.VeryLarge)
			{
				continue;
			}

			int weight = 0;  // This method is used to get bigger airports, so we slightly bias the results toward larger airports, but still allows a couple "smaller" ones
			...
			weightedRoutes.Add(new RouteContainer(new Route(airport.id, PlayerAirport.id,
				(float)Utils.GetDistanceBetweenCoordinates(airport.latitude, airport.longitude, PlayerAirport.latitude, PlayerAirport.longitude))), weight);
		}

		if (weightedRoutes.Count < LargeAirportsToGenerateInCountry)
		{
			result = new SortedSet<RouteContainer>(weightedRoutes);
			largeRoutesByCountry[country] = result;
			return result;
		}

        for (int i = 0; i < LargeAirportsToGenerateInCountry; i++)
        {
			result.Add(weightedRoutes.Next());
        }

		largeRoutesByCountry[country] = result;
        return new SortedSet<RouteContainer>(result);
	}

	private void MakeDictionariesEct()
	{
	...
	// For now nothing below here is used! If my new system works, these will be depreciated
